...
import {
    ProjectProps,
    PROJECT_DEFAULT_STATE,
} from '../../../Store/Reducers/ponikar/projects/projects.types';
    ...
    const submitProject = async () => {
        try {
            ...
            if (!toggleForm) {
                ...
                if (site_url && !isValidURL(project.site_url))
                    throw new Error('Please Enter valid Site URL!');
                if (github_url && !isValidURL(project.github_url))
                    throw new Error('Please Enter valid Github URL!');
                    ...
                    <div className="w-10/12 mx-auto">
                        {!toggleForm && (
                            <>
                                <TextInput
                                    value={title}
                                    onChange={(e) => setAnyProps('title', e)}
                                    label="Project name*"
                                />
                                <TextInput
                                    value={site_url}
                                    type="url"
                                    onChange={(e) => setAnyProps('site_url', e)}
                                    label="Project Link"
                                />
                                <TextInput
                                    value={github_url}
                        ...
                        {isLoading ? (
                            ...
                            <PrimaryButton
                                ...
                                title={toggleForm ? 'Submit' : 'Add to Images'}
