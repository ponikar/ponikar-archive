...
const ProjectPreviewImage :FC<ProjectPreviewProps> = ({ images, setImages  }) => {
   ...
   return <div>
       ...
       { images.length < 6 &&  <MediumButton onClick={openImages} title="Select Images" /> }
        <div className="grid mt-3 grid-cols-3 gap-3">
        { images.map((image, index) => <img onDoubleClick={_ => deleteImage(image)} key={index} src={image} className="object-contain w-full" />) }
        </div>
    </div>
}
