...
const ProjectItem: FC<ProjectItemProps> = ({
    children,
    projectDeletedStarted,
    ...props
}) => {
    const { id, title, description } = props;

    return (
        <section className="bg-white font-primary rounded-primary">
            <ShowcaseItem {...props} />
            <div className="flex items-center p-2 justify-between">
                <h3 className="text-base"> {title} </h3>
                <div className="flex  items-center">
                    <Trash
                        onClick={(_) => projectDeletedStarted(props)}
                        {...iconProps}
                        size={18}
                    />
                    <FancyLink href={`/ponikar/projects/${id}`}>
                        {' '}
...
const mapStateToDispatch = (dispatch) => ({
    projectDeletedStarted: (payload) =>
        dispatch(projectDeletedStarted(payload)),
});
