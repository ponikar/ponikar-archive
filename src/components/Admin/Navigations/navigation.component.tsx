...
const AdminNavigaton:FC<{}> = () => {
    ...
    return <motion.nav  animate={!toggle ? "closed" : "opened"} {...navAnimatedProps} className="fixed font-primary left-0 top-0 h-full w-56 border-r border-grey  bg-back">
        ...
        { toggle && <div className="mt-5 flex w-full items-center justify-between px-4 py-2">
            <Logo />
            <X onClick={_ => setToggle(false)} className="cursor-pointer" size={22} />
        </div> }
        ...
        { toggle && <h4 className="ml-5"> Navigations </h4> }
            <ul className="">
               { adminNavigationData.map(navigation => <AdminNavigationLink key={navigation.href} toggle={toggle} {...navigation} />) }
            </ul>
        </div>
    </motion.nav>
};
